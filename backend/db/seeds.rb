...
# 個別に作成する場合は: rails db:seed:generation_logic_test
if ENV['INCLUDE_GENERATION_LOGIC_TEST'] == 'true' || ENV['INCLUDE_GENERATION_LOGIC_TEST'] == '1'
  puts '===== 習慣化タスク生成ロジック検証用データ ====='
  puts ''
  ...
  generation_test_1 = RoutineTask.find_or_create_by(
    account_id: test_user_id,
    ...
    rt.interval_value = nil
    rt.start_generation_at = 5.days.ago
    rt.next_generation_at = today_beginning
    rt.last_generated_at = 1.day.ago
    rt.max_active_tasks = 3
    rt.category_id = categories_map['健康']&.id
    rt.priority = 'high'
  ...
  4.times do |i|
    ...
    Task.find_or_create_by(
      account_id: test_user_id,
      routine_task_id: generation_test_1.id,
      generated_at: task_date
    ) do |t|
      t.title = generation_test_1.title
      t.due_date = task_date.to_date + 1.day
      t.status = 'completed'
      t.priority = generation_test_1.priority
      t.category_id = generation_test_1.category_id
    end
  end

  puts "  RoutineTask作成: #{generation_test_1.title} (ID: #{generation_test_1.id})"
  puts "  完了済みタスク: 4個、未完了タスク: 0個、上限: 3"
  ...
  generation_test_2 = RoutineTask.find_or_create_by(
    account_id: test_user_id,
    ...
    rt.interval_value = nil
    rt.start_generation_at = 3.days.ago
    rt.next_generation_at = today_beginning + 6.hours
    rt.last_generated_at = 1.day.ago
    rt.max_active_tasks = 5
    rt.category_id = categories_map['健康']&.id
    rt.priority = 'medium'
  ...
  puts "  RoutineTask作成: #{generation_test_2.title} (ID: #{generation_test_2.id})"
  puts "  next_generation_at: #{generation_test_2.next_generation_at.in_time_zone('Tokyo')}"
  ...
  generation_test_3 = RoutineTask.find_or_create_by(
    account_id: test_user_id,
    title: '[検証] 重複生成防止のテスト'
  ) do |rt|
    rt.frequency = 'daily'
  ...
  Task.find_or_create_by(
    account_id: test_user_id,
    routine_task_id: generation_test_3.id,
    generated_at: today_beginning
  ) do |t|
    t.title = generation_test_3.title
    t.due_date = today + 1.day
    t.status = 'completed'
    t.priority = generation_test_3.priority
    t.category_id = generation_test_3.category_id
  end
  ...
  puts "  RoutineTask作成: #{generation_test_3.title} (ID: #{generation_test_3.id})"
  puts "  今日のタスク（完了済み）: 1個、昨日のタスク（未完了）: 1個"
  puts "  検証ポイント: 既存のタスク（完了済み含む）のgenerated_atをチェックして重複生成を防ぐこと"
  puts ''

  # ケース4: タスクが生成されない場合の更新チェック
  puts 'ケース4: タスクが生成されない場合の更新チェック'
  generation_test_4 = RoutineTask.find_or_create_by(
    account_id: test_user_id,
    title: '[検証] タスク生成なしの場合の更新チェック'
  ) do |rt|
    rt.frequency = 'daily'
    rt.interval_value = nil
    rt.start_generation_at = 1.day.from_now
  ...
  puts "  RoutineTask作成: #{generation_test_4.title} (ID: #{generation_test_4.id})"
  puts "  start_generation_at: #{generation_test_4.start_generation_at.in_time_zone('Tokyo')} (未来)"
  puts "  検証ポイント: タスクが生成されない場合、last_generated_atとnext_generation_atが更新されないこと"
  puts ''

  puts '====================================='
  puts '検証方法:'
  puts "  ケース1: RoutineTaskGeneratorJob.perform_now(#{generation_test_1.id}, SecureRandom.uuid)"
  puts "  ケース2: RoutineTaskGeneratorJob.perform_now(#{generation_test_2.id}, SecureRandom.uuid)"
  puts "  ケース3: RoutineTaskGeneratorJob.perform_now(#{generation_test_3.id}, SecureRandom.uuid)"
  puts "  ケース4: RoutineTaskGeneratorJob.perform_now(#{generation_test_4.id}, SecureRandom.uuid)"
  puts '====================================='
  puts ''
end
